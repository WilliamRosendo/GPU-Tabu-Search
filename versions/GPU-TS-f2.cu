...
int *d_sorted_solution;     // current solution ordered using the vertex gains
int *d_bestSolution;        // best solution found
int *d_tab;             // tabu list
int *d_pos;             // position of the vertices in the current solution
int *d_reduced_vertices;    // vector with nbr_delta_v_reduced and nbr_v_reduced respectively
char *d_selected_vertices;   // boolean vector of vertices that meet the thresholds
double *d_edge_gain;        // wswap vector
double *d_gain;         // swap move gains
double *d_vertex_gain;      // gains of each vertex
double *d_minmax_vertex_gain;
...
void *d_temp_storage = NULL, *d_temp_storage2 = NULL, *d_temp_storage3 = NULL, *d_temp_storage4 = NULL, *d_temp_storage5 = NULL;
size_t temp_storage_bytes = 0, temp_storage_bytes2 = 0, temp_storage_bytes3 = 0, temp_storage_bytes4 = 0, temp_storage_bytes5 = 0;
...
void setup_data()
{
    ...
    cudaMalloc((void**)&d_sorted_solution, total_nbr_v * sizeof(int));
    cudaMalloc((void**)&d_bestSolution, nbr_v * sizeof(int));
    cudaMalloc((void**)&d_vertex_gain, total_nbr_v * sizeof(double));
    cudaMalloc((void**)&d_minmax_vertex_gain, 2 * sizeof(double));
    cudaMalloc((void**)&d_selected_vertices, total_nbr_v * sizeof(char));
    cudaMalloc((void**)&d_reduced_vertices, 2 * sizeof(int));
...
double calculateVertexGain(int* currentSolution, double *vertexGain, bool* isSolution, int* pos){
    ...
    for(int i = 0; i < nbr_v; i++){
        ...
        for(int j = CSR_ROWPTR[vertex]; j < CSR_ROWPTR[vertex+1]; j++){
            int position = pos[CSR_COLIND[j]];
            vertexGain[position] += CSR_VAL[j];
...
__global__ void updateVertexEdgeGain(int* d_sorted_solution, double* d_vertex_gain, double* d_edge_gain, int *d_pos, int* d_CSR_R, int* d_CSR_COL, double* d_CSR_VAL, int index_u, int index_v, int nbr_v, int delta_nbr_v){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if(idx < qnt_u){
        ...
        int pos = d_pos[z];
        atomicAdd2(&d_vertex_gain[pos], -edge_weight);

        if (z != v) {

            if (d_pos[z] >= nbr_v) {
                int gain_idx = d_pos[u] * delta_nbr_v + d_pos[z] - nbr_v;
                atomicAdd2(&d_edge_gain[gain_idx], -edge_weight);
            } else {
                int gain_idx = d_pos[z] * delta_nbr_v + d_pos[v] - nbr_v;
                atomicAdd2(&d_edge_gain[gain_idx], edge_weight);
            }
        }
    }
...
__global__ void updateSolution(int* d_crrntSolution, int* d_sorted_solution, double *d_vertex_gain, int* d_pos, int* d_tab, int index_u, int index_v, int C1, int C2, int iter){
    int u = d_sorted_solution[index_u];
    int v = d_sorted_solution[index_v];
    d_tab[u] = iter + C1;
    d_tab[v] = iter + C2;
    ...
    double tmp_gain = d_vertex_gain[pos_u];
    d_vertex_gain[pos_u] = d_vertex_gain[pos_v];
    d_vertex_gain[pos_v] = tmp_gain;
...
// select vertex
__global__ void selectVertices(int* d_crrntSolution, double* d_vertex_gain, char* d_selected_vertices, double lower_bound, double upper_bound, int nbr_v, int total_nbr_v) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < total_nbr_v) {
        if (idx < nbr_v) {
            d_selected_vertices[idx] = d_vertex_gain[idx] <= lower_bound ? 1 : 0;
        } else {
            d_selected_vertices[idx] = d_vertex_gain[idx] >= upper_bound ? 1 : 0;
        }
    }
}

// calculate swap gains
__global__ void calculateSwapGain(int* d_sorted_solution, double* d_vertex_gain, double* d_gain, double* d_edge_gain, int *d_pos, int *d_tab, int iter, double currentWeight, double bestWeight, int* d_CSR_R, int* d_CSR_COL, double* d_CSR_VAL, int nbr_v, int nbr_v_reduced, int delta_nbr_v, int delta_nbr_v_reduced, int nbr_movs_reduced){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    if(idx < nbr_movs_reduced){
        int idx_u = idx/delta_nbr_v_reduced;
        int idx_v = (idx%delta_nbr_v_reduced) + nbr_v;

        int u = d_sorted_solution[idx_u];
        int v = d_sorted_solution[idx_v];

        int pos_u = d_pos[u];
        int pos_v = d_pos[v];
        int pos_uv = pos_u * delta_nbr_v + pos_v - nbr_v;
        d_gain[idx] = d_vertex_gain[pos_v] - d_vertex_gain[pos_u] - d_edge_gain[pos_uv];

        if( (d_tab[u] > iter || d_tab[v] > iter) && (currentWeight + d_gain[idx] <= bestWeight) ){
            d_gain[idx] = -2*BIG_VALUE + d_gain[idx];
        }
    }
}
...
void tabu_search(int *sol){
    ...
    int pos[total_nbr_v];
    int h_reduced_vertices[2];
    double h_maxmin_vertex_gain[2];
    ...
    // memory allocation
    if(temp_storage_bytes == 0 || temp_storage_bytes2 == 0 || temp_storage_bytes3 == 0 || temp_storage_bytes4 == 0 || temp_storage_bytes5 == 0) {
        gpuErrchk( cub::DeviceReduce::ArgMax(d_temp_storage, temp_storage_bytes, d_gain, d_out, nbr_movs) );
        gpuErrchk( cub::DeviceReduce::Max(d_temp_storage2, temp_storage_bytes2, d_vertex_gain + nbr_v, d_minmax_vertex_gain, delta_nbr_v) );
        gpuErrchk( cub::DeviceReduce::Min(d_temp_storage3, temp_storage_bytes3, d_vertex_gain, d_minmax_vertex_gain + 1, nbr_v) );
        gpuErrchk( cub::DeviceSelect::Flagged(d_temp_storage4, temp_storage_bytes4, d_crrntSolution + nbr_v, d_selected_vertices + nbr_v, d_sorted_solution + nbr_v, d_reduced_vertices, delta_nbr_v) );
        gpuErrchk( cub::DeviceSelect::Flagged(d_temp_storage5, temp_storage_bytes5, d_crrntSolution, d_selected_vertices, d_sorted_solution, d_reduced_vertices + 1, nbr_v));

        gpuErrchk( cudaMalloc(&d_temp_storage, temp_storage_bytes) );
        gpuErrchk( cudaMalloc(&d_temp_storage2, temp_storage_bytes2) );
        gpuErrchk( cudaMalloc(&d_temp_storage3, temp_storage_bytes3) );
        gpuErrchk( cudaMalloc(&d_temp_storage4, temp_storage_bytes4) );
        gpuErrchk( cudaMalloc(&d_temp_storage5, temp_storage_bytes5) );
    }

    int deg = nbr_v * total_nbr_v;
    int blcosksEdgeGain = deg / 256 + 1;
    int blocksVertexEdgeGain = total_nbr_v / 256 + 1;
    ...
    cudaStream_t sort_stream[2];
    cudaStreamCreate(&sort_stream[0]);
    cudaStreamCreate(&sort_stream[1]);

    int iter = 0;
    while(iter < max_iter){
        gpuErrchk( cub::DeviceReduce::Max(d_temp_storage2, temp_storage_bytes2, d_vertex_gain + nbr_v, d_minmax_vertex_gain, delta_nbr_v, sort_stream[0]) );
        gpuErrchk( cub::DeviceReduce::Min(d_temp_storage3, temp_storage_bytes3, d_vertex_gain, d_minmax_vertex_gain + 1, nbr_v, sort_stream[1]) );

        gpuErrchk( cudaMemcpy(h_maxmin_vertex_gain, d_minmax_vertex_gain, 2 * sizeof(double), cudaMemcpyDeviceToHost) );

        double upper_bound = h_maxmin_vertex_gain[0] - scale_factor * max_diversity;
        double lower_bound = h_maxmin_vertex_gain[1] + scale_factor * max_diversity;
        selectVertices<<<blocksVertexEdgeGain, 256>>>(d_crrntSolution, d_vertex_gain, d_selected_vertices, lower_bound, upper_bound, nbr_v, total_nbr_v);

        cub::DeviceSelect::Flagged(d_temp_storage4, temp_storage_bytes4, d_crrntSolution + nbr_v, d_selected_vertices + nbr_v, d_sorted_solution + nbr_v, d_reduced_vertices, delta_nbr_v, sort_stream[0]);
        cub::DeviceSelect::Flagged(d_temp_storage5, temp_storage_bytes5, d_crrntSolution, d_selected_vertices, d_sorted_solution, d_reduced_vertices + 1, nbr_v, sort_stream[1]);

        gpuErrchk( cudaMemcpy(&h_reduced_vertices, d_reduced_vertices, 2*sizeof(int), cudaMemcpyDeviceToHost) );
        int nbr_delta_v_reduced = h_reduced_vertices[0];
        int nbr_v_reduced = h_reduced_vertices[1];
        int nbr_movs_reduced = nbr_delta_v_reduced * nbr_v_reduced;
        total_evaluations += nbr_movs_reduced;

        int blocksSwapGain = nbr_movs_reduced / 256 + 1;
        calculateSwapGain<<<blocksSwapGain, 256>>>(d_sorted_solution, d_vertex_gain, d_gain, d_edge_gain, d_pos, d_tab, iter,
                                                   current_weight, bestWeight, d_CSR_R, d_CSR_COL, d_CSR_VAL, nbr_v,
                                                   nbr_v_reduced, delta_nbr_v, nbr_delta_v_reduced, nbr_movs_reduced);
        gpuErrchk( cudaPeekAtLastError() );

        // choose the swap with the highest gain
        gpuErrchk( cub::DeviceReduce::ArgMax(d_temp_storage, temp_storage_bytes, d_gain, d_out, nbr_movs_reduced) );
        gpuErrchk( cudaMemcpy(&h_out, d_out, sizeof(cub::KeyValuePair <int, double>), cudaMemcpyDeviceToHost) );

        int idx = h_out.key;
        int index_u = idx / nbr_delta_v_reduced;
        int index_v = (idx % nbr_delta_v_reduced) + nbr_v;
        if (h_out.value < -BIG_VALUE ) {
            current_weight += h_out.value + 2*BIG_VALUE;
        } else {
        ...
        updateVertexEdgeGain<<<blocksVertexEdgeGain, 256>>>(d_sorted_solution, d_vertex_gain, d_edge_gain, d_pos, d_CSR_R, d_CSR_COL, d_CSR_VAL, index_u, index_v, nbr_v,
                                                            delta_nbr_v);
        gpuErrchk( cudaPeekAtLastError() );

        // updates the current solution by swapping
        updateSolution<<<1, 1>>>(d_crrntSolution, d_sorted_solution, d_vertex_gain, d_pos, d_tab, index_u,
                                 index_v, C1, C2, iter);
        gpuErrchk( cudaPeekAtLastError() );
    ...
    cudaStreamDestroy(sort_stream[0]);
    cudaStreamDestroy(sort_stream[1]);
