...
int nbr_movs;
int nbr_movs_reduced;
int nbr_v_reduced;
float nbr_v_rfactor = 0.3;
int delta_nbr_v_reduced;
float delta_nbr_v_rfactor = 0.3;
...
double *d_sorted_vertex_gain;       // sorted gains of each vertex
...
void read_instance()
{
    ...
    nbr_v_reduced = nbr_v * nbr_v_rfactor;
    delta_nbr_v_reduced = delta_nbr_v * delta_nbr_v_rfactor;
    nbr_movs_reduced = nbr_v_reduced * delta_nbr_v_reduced;
...
void setup_data()
{
    ...
    cudaMalloc((void**)&d_sorted_vertex_gain, total_nbr_v * sizeof(double));
...
void tabu_search(int *sol){
    ...
    if(temp_storage_bytes2 == 0){
        gpuErrchk( cub::DeviceRadixSort::SortPairsDescending(d_temp_storage2, temp_storage_bytes2, d_vertex_gain + nbr_v, d_sorted_vertex_gain + nbr_v, d_crrntSolution + nbr_v, d_sorted_solution + nbr_v, delta_nbr_v) );
        gpuErrchk( cudaMalloc(&d_temp_storage2, temp_storage_bytes2) );
    }

    if(temp_storage_bytes3 == 0){
        gpuErrchk( cub::DeviceRadixSort::SortPairs(d_temp_storage3, temp_storage_bytes3, d_vertex_gain, d_sorted_vertex_gain, d_crrntSolution, d_sorted_solution, nbr_v) );
    ...
    int iter = 0;
    while(iter < max_iter){
        int *d_input_solution;

        if (iter > (max_iter * 2/3)) {
            gpuErrchk( cub::DeviceRadixSort::SortPairsDescending(d_temp_storage2, temp_storage_bytes2, d_vertex_gain + nbr_v, d_sorted_vertex_gain + nbr_v, d_crrntSolution + nbr_v, d_sorted_solution + nbr_v, delta_nbr_v, 0, sizeof(double)*8, sort_stream[0]) );
            gpuErrchk( cub::DeviceRadixSort::SortPairs(d_temp_storage3, temp_storage_bytes3, d_vertex_gain, d_sorted_vertex_gain, d_crrntSolution, d_sorted_solution, nbr_v, 0, sizeof(double)*8, sort_stream[1]) );

            d_input_solution = d_sorted_solution;
        
            nbr_v_reduced = nbr_v * nbr_v_rfactor;
            delta_nbr_v_reduced = delta_nbr_v * delta_nbr_v_rfactor;
            nbr_movs_reduced = nbr_v_reduced * delta_nbr_v_reduced;
        } else {
            d_input_solution = d_crrntSolution;

            nbr_v_reduced = nbr_v;
            delta_nbr_v_reduced = delta_nbr_v;
            nbr_movs_reduced = nbr_v * delta_nbr_v;

        }
        int blocksSwapGain = nbr_movs_reduced / 256 + 1;
        calculateSwapGain<<<blocksSwapGain, 256>>>(d_input_solution, d_vertex_gain, d_gain, d_edge_gain, d_pos, d_tab, iter,
                                                          current_weight, bestWeight, d_CSR_R, d_CSR_COL, d_CSR_VAL, nbr_v,
                                                          nbr_v_reduced, total_nbr_v, delta_nbr_v, delta_nbr_v_reduced, nbr_movs_reduced);
        ...
        int idx = h_out.key;
        int index_u = idx / delta_nbr_v_reduced;
        int index_v = (idx % delta_nbr_v_reduced) + nbr_v;
        ...
        updateVertexEdgeGain<<<blocksVertexEdgeGain, 256>>>(d_input_solution, d_vertex_gain, d_edge_gain, d_pos, d_CSR_R, d_CSR_COL, d_CSR_VAL, index_u, index_v, nbr_v,
                                                            delta_nbr_v);
        gpuErrchk( cudaPeekAtLastError() );

        // updates the current solution by swapping
        updateSolution<<<1, 1>>>(d_crrntSolution, d_input_solution, d_vertex_gain, d_pos, d_tab, index_u,
                                 index_v, C1, C2, iter);
        gpuErrchk( cudaPeekAtLastError() );
